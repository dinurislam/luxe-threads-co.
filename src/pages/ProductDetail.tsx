    ...
    <main className="pt-20 min-h-screen">
      <div className="flex flex-col lg:flex-row">
        <div className="lg:w-[60%]">
          <motion.div key={selectedImage} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, ease: [0.2, 0, 0, 1] }} className="aspect-[3/4] bg-secondary">
          ...
          {product.images.length > 1 && (
            <div className="flex gap-px mt-px">
              {product.images.map((img, i) => (
                <button key={i} onClick={() => setSelectedImage(i)} className={`flex-1 aspect-[3/4] bg-secondary overflow-hidden ${i === selectedImage ? "ring-1 ring-foreground" : ""}`}>
        ...
        <div className="lg:w-[40%] px-6 lg:px-12 py-8 lg:py-16 lg:sticky lg:top-16 lg:self-start">
          ...
          <div className="mb-8">
            ...
            {!selectedSize && <p className="text-[10px] text-muted-foreground mt-2">Please select a size</p>}
          </div>

          <div className="flex flex-col gap-3">
            <button onClick={handleAddToCart} disabled={!selectedSize || !inStock} className="w-full bg-foreground text-background text-[11px] uppercase tracking-premium py-4 font-body hover:bg-foreground/90 ghost-slide disabled:opacity-40 disabled:cursor-not-allowed">
              Add to Bag
            </button>
            <button onClick={handleBuyNow} disabled={!selectedSize || !inStock} className="w-full border border-foreground text-foreground text-[11px] uppercase tracking-premium py-4 font-body hover:bg-foreground hover:text-background ghost-slide disabled:opacity-40 disabled:cursor-not-allowed">
              Buy Now
            </button>
          </div>
        </div>
